...
@dataclass(frozen=True)  # Make AuxRule hashable
class AuxRule:
    pattern: str
    proto_tags: Set[str]
    actions: Set[str]
...
class MitochondrialFeedbackEngine:
    ...
    def compose_intervals(self, genome: str) -> List[Tuple[int, int]]:
        ...
        for key, group in groupby(annotations, key=lambda x: tuple(sorted(x[2].proto_tags))):
    ...
    def score_clone(self, genome: str, clone_regions: List[Tuple[int, int]]) -> Dict[str, Any]:
        ...
        phi_clone = set()
        rules_in_clone = set()  # Now works with frozen/hashable AuxRule
        ...
        for start, end in clone_regions:
            ...
            for rule in self.aux_table:
                if rule.compiled_pattern.search(region_seq):
                    ...
                    rules_in_clone.add(rule)  # Now works with frozen/hashable AuxRule
    ...
    def update_aux_table(self, certificate: Dict[str, Any]) -> None:
        ...
        # Create a new list for updated rules
        updated_rules = []
        
        for rule in self.aux_table:
            # Check if this rule needs to be updated
            new_rule = AuxRule(
                pattern=rule.pattern,
                proto_tags=rule.proto_tags.copy(),
            ...
            # Apply updates from recommendations
            for rec in certificate["recommendations"]:
                if "increase_penalty" in rec:
                    for update in rec["increase_penalty"]:
                        if update["rule"] == rule.pattern:
                            object.__setattr__(new_rule, 'penalty', new_rule.penalty + update["delta"])
                            print(f"Updated penalty for {rule.pattern} to {new_rule.penalty}")
                
                if "deprioritize" in rec:
                    for update in rec["deprioritize"]:
                        if update["rule"] == rule.pattern:
                            object.__setattr__(new_rule, 'priority', update["new_priority"])
                            print(f"Updated priority for {rule.pattern} to {new_rule.priority}")
            
            updated_rules.append(new_rule)
        
        # Replace the aux table with updated rules
        self.aux_table = updated_rules
        self.aux_table.sort(key=lambda x: x.priority, reverse=True)
